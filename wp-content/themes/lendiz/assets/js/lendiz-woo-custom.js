( function( $ ) {
	...
	$(document).ready(function(){
		...
		if( $( document ).find('.mini-cart-item').length ){
			...
			$( document ).on( 'click', '.remove-cart-item', function(){
				...
				$.ajax({
					...
					data: { 
						action: "lendiz_product_remove", 
						nonce: lendiz_ajax_var.remove_from_cart,
		...
		$( document ).on( 'click', "a.lendiz_ajax_add_to_cart", function( event) {
			
			if( $("body.page-template-lendiz-wishlist").length ){
				$(this).parents("tr").find("a.lendiz-wishlist-remove").trigger("click");
			}
			...
			$.ajax({
				...
				url: lendiz_ajax_var.admin_ajax_url,
				data: { 
					action: "lendiz_add_to_cart",
					product_id: product_id,
					nonce: lendiz_ajax_var.add_to_cart
				},success: function(data){
					cur_ele.removeClass("loading");
					if( data['status'] == 1 ){							
						if( data['mini_cart'] ){
