...
if( class_exists( "LendizCPT" ) ){
	if( is_singular( array( 'lendiz-portfolio', 'lendiz-team', 'lendiz-testimonial', 'lendiz-events', 'lendiz-services' ) ) ){
...
$full_width_class = '';
// Post View Count
$aps->lendiz_set_post_view_count( get_the_ID() );
?>
<div class="lendiz-content <?php echo esc_attr( 'lendiz-' . $template ); ?>">
	...
	<?php 
		if( $aps->lendiz_check_meta_value( 'lendiz_post_featured_slider', $template.'-featured-slider' ) ){
			$ahe->lendiz_featured_slider( $template );
		}
	?>
	
	<?php
		if( $aps->lendiz_check_meta_value( 'lendiz_post_full_wrap', 'single-post-full-wrap' ) ) : 
	?>
			<div class="post-full-thumb-wrap single-post-template">
				<?php
					if ( has_post_format( 'gallery' ) ) :
						$aps->lendiz_gallery_format();
				?>
				<?php elseif ( has_post_format( 'video' ) ) :
					...
					$video_modal = $aps->lendiz_check_meta_value( 'lendiz_post_video_modal', $template.'-video-format' );
					$video_atts = array(
							...
							<div class="post-overlay-items<?php echo ( has_post_format( 'image' ) ? ' thumb-exists' : '' ); ?>"> 
								<?php
									...
									if( $post_oitems_opt == '' || $post_oitems_opt == 'theme-default' ){
										$post_elements = LendizThemeOpt::lendiz_static_theme_mod( 'single-post-overlay-items' );		
									...
									$aps->lendiz_post_overlay_items( $post_elements );
								?>
							</div>
						</div>
					</div>
				...
				<div class="<?php echo esc_attr( $template_class['content_class'] ); ?>">
					...
					<div id="primary" class="content-area">
						...
						<?php
							...
							$post_elements = array();
							$post_oitems_opt = get_post_meta( get_the_ID(), 'lendiz_post_page_items_opt', true );
							if( $post_oitems_opt == '' || $post_oitems_opt == 'theme-default' ){
								$post_elements = LendizThemeOpt::lendiz_static_theme_mod( 'single-post-page-items' );	
								$post_elements = isset( $post_elements['Enabled'] ) ? $post_elements['Enabled'] : '';		
							}else{
								$overlay_post_items = get_post_meta( get_the_ID(), 'lendiz_post_page_items', true );
								$post_elements = json_decode( stripslashes( $overlay_post_items ), true );
						...
						<div class="post-navigation-wrapper clearfix">
							<nav class="navigation post-navigation">
								<div class="nav-links custom-post-nav">
									...
									if (!empty( $prev_post )): ?>
										<?php
											$prev_title = $prev_post->post_title ? $prev_post->post_title : esc_html__( "Previous Post" , "lendiz" );
										?>
										<div class="prev-nav-link">
											<i class="ti-arrow-left"></i><span class="post-nav-text"><?php esc_html_e( "Prev" , "lendiz" ); ?></span>
									...
									if (!empty( $next_post )): ?>
										<?php
											$next_title = $next_post->post_title ? $next_post->post_title : esc_html__( "Next Post" , "lendiz" );
										?>
										<div class="next-nav-link">
											<span class="post-nav-text"><?php esc_html_e( "Next" , "lendiz" ); ?></span><i class="ti-arrow-right"></i>
						...
						<?php 
							$comment_type = LendizThemeOpt::lendiz_static_theme_mod( 'comments-type' );
							if( $comment_type == 'wp' ){
