...
( function( $ ) {
	...
	$(document).on( 'click', '.tgm-custom-plugin-install', function(e) {
		...
		if( $(".install-plugins-parent > .bulk-action-active").length ){
			...
			var plugins = lendiz_admin_ajax_var.lendiz_plugins;

			lendiz_plugin_install( 0, plugins, parent_div );
			
		}//if any plugin selected
		
	});
	
	function lendiz_plugin_install( index, plugins, parent_div ){
		...
		if( max_len > index ){
			...
			$.ajax({
				...
				url: ajaxurl,
				data: {
					'action' : 'lendiz_tgm_install',
					'plugin' : plugins_arr[index],
					'tgmpa-install' : 'install-plugin',
					'lendiz_tgmpa_nounce' : lendiz_admin_ajax_var.lendiz_tgmpa_nounce,
					'plugins' : plugins
				},
				success: function(response){
					index++;
					lendiz_plugin_install( index, plugins, parent_div );
				},
			...
			window.location = lendiz_admin_ajax_var.lendiz_installation_url + "&lendiz_plugin_actv="+ plugins_str;
		}
	}	
	...
	$(document).on( 'click', '.button-uninstall-demo', function(e) {
		...
		$.confirm({
			theme: 'supervan',
			title: false,
			content: lendiz_admin_ajax_var.unins_confirm,
			confirmButtonClass: 'btn-success',
			cancelButtonClass: 'btn-danger',
			confirmButton: lendiz_admin_ajax_var.yes,
   			cancelButton: lendiz_admin_ajax_var.no,
			confirm: function(){
				...
				progress_text.text(lendiz_admin_ajax_var.uninstalling);
				loading_wrap.show();
				...
				$.ajax({
					...
					url: ajaxurl,
					data: {
						action: 'lendiz_uninstall',
						nounce:  lendiz_admin_ajax_var.lendiz_demo_nounce
					},
					success: function(response){
						if( response && response.indexOf('success') == -1 ) {
							...
							loading_wrap.hide();
							progress_text.text(lendiz_admin_ajax_var.uninstalled);
							window.location = location.href;
						}
					},
					error: function(response, errorThrown){
						loading_wrap.hide();
						alert(lendiz_admin_ajax_var.unins_pbm);
					}
				});
			}
		});
	...
	var zozo_admin_screen = {
		...
		install_demos: function() {
			$(document).on( 'click', '.button-install-demo', function(e) {
				...
				var req_plugins_stat = $("#lendiz-required-plugins-stat").val();
				var proceed_btn = req_plugins_stat == '1' ? lendiz_admin_ajax_var.proceed : '';
				if( req_plugins_stat != '1' ){
					requirement = '<h2 class="lendiz-blink">You must install and activate required plugins.</h2>'+requirement;
				}
				...
				if( choosed_demo !== null ) {
					...
					$.confirm({
						...
						confirmButton: proceed_btn,
   						cancelButton: lendiz_admin_ajax_var.cancel,
						confirm: function(){
							...
							if( req_plugins_stat == '1' ){
								...
								loading_wrap.show();
								$(current).parents('.zozothemes-demo-item').find('.installation-progress p').text(lendiz_admin_ajax_var.downloading);
								$('.zozo-importer-notice').hide();
							
								/*Demo Files Download*/
								lendiz_check_file_access_permission( choosed_demo, revslider, progress, progress_text );
	...
	function lendiz_progress_update( progress ){
	...
	function lendiz_progress_end( choosed_demo, progress ){
		...
		$.confirm({
			...
			confirmButton: 'Ok',
			cancelButton: 'Close',
			confirm: function(){ window.location = lendiz_admin_ajax_var.lendiz_installation_url; }
		});
	}
	
	function lendiz_progress_details( progress, msg, stat ){
	...
	function lendiz_check_file_access_permission( choosed_demo, revslider, progress, progress_text ){
		$.ajax({
			type: 'post',
			dataType: 'json',
			url: ajaxurl,
			data: {
				action: 'lendiz_chk_prmsn',
				nounce:  lendiz_admin_ajax_var.lendiz_demo_nounce
			},
			success: function(response){
				if( response['msg'] == 'failed' ) {
					progress_text.text(response['msg']); lendiz_progress_details( progress, response['msg'], '' );
					lendiz_progress_end( choosed_demo, progress );					
				}else{
					lendiz_general_file_download_function( 0, choosed_demo, revslider, progress, progress_text );
				}
			},
			error: function(response, errorThrown){
				progress_text.text(response['msg']); lendiz_progress_details( progress, response['msg'], 'failed' );
				lendiz_progress_end( choosed_demo, progress );
			}
		});//ajax end		
	}
	
	function lendiz_general_file_download_function( index, choosed_demo, revslider, progress, progress_text ){
		...
		if( max_len > index ){
			$.ajax({
				...
				success: function(response){
					...
					if( max_len > index ){
						lendiz_general_file_download_function( index, choosed_demo, revslider, progress, progress_text );
					}else{
						lendiz_page_file_download_function( 0, choosed_demo, revslider, progress, progress_text );
					}
					lendiz_progress_update( progress );
				},
				error: function(response, errorThrown){
					lendiz_progress_details( progress, response['msg'], 'failed' );
					index += 1;
					if( max_len > index ){
						lendiz_general_file_download_function( index, choosed_demo, revslider, progress, progress_text );
					}else{
	...
	function lendiz_page_file_download_function( index, choosed_demo, revslider, progress, progress_text ){
		...
		if( max_len > index ){
			$.ajax({
				...
				url: ajaxurl,
				data: {
					action: 'lendiz_download_parts_xml',
					nounce:  lendiz_admin_ajax_var.lendiz_demo_nounce,
					demo_type: choosed_demo,
					revslider: revslider,
					key: page_arr[index],
				...
				success: function(response){
					...
					if( max_len > index ){
						lendiz_page_file_download_function( index, choosed_demo, revslider, progress, progress_text );
					}else{
						lendiz_general_file_install_function( 0, choosed_demo, revslider, progress, progress_text );
					}
					lendiz_progress_update( progress );
				},
				error: function(response, errorThrown){
		...
		if( max_len == 0 ){
			lendiz_general_file_install_function( index, choosed_demo, revslider, progress, progress_text );
		}
		
	}
	
	function lendiz_general_media_install_function( index, choosed_demo, revslider, progress, progress_text, parent_index ){
		...
		if( max_len > index ){
			$.ajax({
				...
				success: function(response){
					...
					index += 1;
					if( max_len > index ){
						lendiz_general_media_install_function( index, choosed_demo, revslider, progress, progress_text, parent_index );
					}else{
						lendiz_progress_update( progress );
						parent_index += 1;
						lendiz_page_file_install_function( 0, choosed_demo, revslider, progress, progress_text );
					}
				},
				error: function(response, errorThrown){
					var msg = general_arr[index] + ' not imported';
					lendiz_progress_details( progress, msg, 'failed' );
					index += 1;
					if( max_len > index ){
						lendiz_general_media_install_function( index, choosed_demo, revslider, progress, progress_text, parent_index );
					}else{
	...
	function lendiz_page_file_install_function( index, choosed_demo, revslider, progress, progress_text ){
		var parent_index = 0;
		var max_len = page_arr.length;
		if( max_len > index ){
			$.ajax({
				...
				url: ajaxurl,
				data: {
					action: 'lendiz_install_parts_xml',
					nounce:  lendiz_admin_ajax_var.lendiz_demo_nounce,
					key: page_arr[index],
					part: 'pages',
					label: page_txt_arr[index]
				...
				success: function(response){
					...
					index += 1;
					if( max_len > index ){
						lendiz_page_file_install_function( index, choosed_demo, revslider, progress, progress_text );
					}else{
						parent_index = media_parts ? 1 : 0;
						lendiz_general_file_install_function( parent_index, choosed_demo, revslider, progress, progress_text );
					}
					lendiz_progress_update( progress );
				},
				error: function(response, errorThrown){
	...
	function lendiz_general_file_install_function( index, choosed_demo, revslider, progress, progress_text ){
		var max_len = general_arr.length;
		if( max_len > index ){
		
			if( general_arr[index] == 'media' ){
				lendiz_general_media_install_function( 0, choosed_demo, revslider, progress, progress_text, index );
			}else{
				...
				$.ajax({
					...
					success: function(response){
						...
						if( max_len > index ){
							...
							lendiz_import_final( choosed_demo, progress, progress_text );
						}
						lendiz_progress_update( progress );
					},
					error: function(response, errorThrown){
	...
	function lendiz_import_final( choosed_demo, progress, progress_text ){
		$.ajax({
			type: 'post',
			dataType: 'json',
			url: ajaxurl,
			data: {
				action: 'lendiz_import_set_default',
				nounce:  lendiz_admin_ajax_var.lendiz_demo_nounce,
				demo_type: choosed_demo
			},
			success: function(response){
				...
				$.confirm({
					...
					confirm: function(){
						...
						window.location = lendiz_admin_ajax_var.lendiz_installation_url;
					},
					cancel: function(){
