<?php
add_action( 'widgets_init', 'lendiz_zozo_contact_info_widget' );
function lendiz_zozo_contact_info_widget() {
	register_widget( 'lendiz_zozo_contact_infos_widget' );
}

class lendiz_zozo_contact_infos_widget extends WP_Widget {
	/**
	 * Widget setup.
	 */
	function __construct() {
		
		//add_action( 'admin_enqueue_scripts', array( $this, 'lendiz_zozo_contact_infos_widget_script' ) );
		add_action( 'load-widgets.php', array( &$this, 'lendiz_zozo_contact_infos_widget_script' ) );
		
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'zozo_contact_info_widget', 'description' => esc_html__('A widget that displays an About widget', 'lendiz-core') );
		/* Widget control settings. */
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'zozo_contact_info_widget' );
		/* Create the widget. */
		parent::__construct( 'zozo_contact_info_widget', esc_html__('Lendiz Contact Info', 'lendiz-core'), $widget_ops, $control_ops );
		
	}
	
	function lendiz_zozo_contact_infos_widget_script(){
			...
			<div class="contact-widget widget-content">
				...
				<div class="contact-widget-info">
					...
					<p class="contact-phone">
						...
						<span>
							<?php
							if( strpos( $cphone, "," ) !== false ) : 
								$phone_each = explode (",", $cphone); $c = 1;
								foreach( $phone_each as $phone_num ){
									if( !$c ) echo ', '; $c = 0;
							?>
								<a href="tel:<?php echo preg_replace('/\s+/', '', $phone_num); ?>"><?php echo esc_html( $phone_num ); ?></a>
							<?php
								}
							else: ?>
								<a href="tel:<?php echo preg_replace('/\s+/', '', $cphone); ?>"><?php echo esc_html( $cphone ); ?></a>
							<?php
							endif;
							?>
						</span>
					...
					<p class="contact-email">
						...
						<span>
							<?php
							if( strpos( $cmail, "," ) !== false ) : 
								$email_each = explode (",", $cmail); $c = 1;
								foreach( $email_each as $email_id ){
									if( !$c ) echo ', '; $c = 0;
							?>
								<a href="mailto:<?php echo preg_replace('/\s+/', '', $email_id); ?>"><?php echo esc_html( $email_id ); ?></a>
							<?php
								}
							else: ?>
								<a href="tel:<?php echo preg_replace('/\s+/', '', $cmail); ?>"><?php echo esc_html( $cmail ); ?></a>
							<?php
							endif;
							?>
						</span>
	...
	function update( $new_instance, $old_instance ) {
		...
		$instance['cmail'] = sanitize_text_field( $new_instance['cmail'] );
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => esc_html__( 'Contact Info', 'lendiz-core' ), 'ctext' => '', 'caddress' => '', 'cphone' => '', 'cmail' => '', 'font_color' => '', 'cfshortcode' => '', 'background_color' => '', 'background_img' => '' );
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'ctext' ) ); ?>"><?php esc_html_e( 'Text Message', 'lendiz-core' ); ?></label>
		...
		<!-- Address -->
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'caddress' ) ); ?>"><?php esc_html_e( 'Address', 'lendiz-core' ); ?></label>
		...
		<!-- Phone Numbers -->
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'cphone' ) ); ?>"><?php esc_html_e( 'Phone Numbers', 'lendiz-core' ); ?></label>
		...
		<!-- Email -->
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'cmail' ) ); ?>"><?php esc_html_e( 'Email', 'lendiz-core' ); ?></label>
		...
		<!-- Shortcode -->
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'cfshortcode' ) ); ?>"><?php esc_html_e( 'Contact Form Shortcode', 'lendiz-core' ); ?></label>
		...
		<!-- Font Color -->		
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'font_color' ) ); ?>"><?php esc_html_e( 'Font Color', 'lendiz-core' ); ?></label>
		...
		<!-- Background Color -->
		<p>
            <label for="<?php echo $this->get_field_id( 'background_color' ); ?>"><?php _e( 'Background Color', 'lendiz-core' ); ?></label>
            <span><?php _e( 'Choose widget background color', 'lendiz-core' ); ?></span>
		...
		<!-- Background Image -->
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'background_img' ) ); ?>"><?php esc_html_e( 'Background Image', 'lendiz-core' ); ?></label>
