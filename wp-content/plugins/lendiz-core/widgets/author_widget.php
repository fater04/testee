...
add_action( 'widgets_init', 'lendiz_zozo_author_load_widget' );

function lendiz_zozo_author_load_widget() {

	register_widget( 'lendiz_zozo_author_widget' );

}

class lendiz_zozo_author_widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_ops = array( 'classname' => 'zozo_author_widget', 'description' => esc_html__('A widget that displays authors details.', 'lendiz-core') );
		...
		parent::__construct( 'zozo_author_widget', esc_html__('Lendiz Author', 'lendiz-core'), $widget_ops, $control_ops );
					...
					foreach ( $blogusers as $user ) {
						...
						$out .= '<li class="clearfix media">
							...
							<div class="author-meta media-body">
								...
								<p class="author-counts"><span><a href="'. esc_url( $author_link ) .'" rel="bookmark">' . count_user_posts( $user->ID ) .' '. esc_html__('Posts', 'lendiz-core') .'</a></span><span>'. $comment_count .' '. esc_html__('Comments', 'lendiz-core') .'</span></p>
	...
	function form( $instance ) {
		...
		<label for="<?php echo esc_attr( $this->get_field_id('listby') ); ?>"><?php esc_html_e('List by:', 'lendiz-core') ?></label> 
		...
		<select id="<?php echo esc_attr( $this->get_field_id('listby') ); ?>" name="<?php echo esc_attr( $this->get_field_name('listby') ); ?>" class="widefat categories" style="width:100%;">
			...
			<option value='default' <?php if ('default' == $instance['listby']) echo 'selected="selected"'; ?>><?php esc_html_e('Default', 'lendiz-core') ?></option>

			<option value='post' <?php if ('post' == $instance['listby']) echo 'selected="selected"'; ?>><?php esc_html_e('No.of Post', 'lendiz-core') ?></option>
		...
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'filter_by' ) ); ?>"><?php esc_html_e('Filter by:', 'lendiz-core'); ?></label>
			...
			<br><small><?php esc_html_e('Filter by - separate by comma(,) eg: administrator,author,editor,subscriber', 'lendiz-core'); ?></small>
		...
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php esc_html_e('Number of author to show:', 'lendiz-core'); ?></label>
