...
$cols = LendizFamework::lendiz_static_theme_mod( 'portfolio-grid-cols' );
$gutter = LendizFamework::lendiz_static_theme_mod( 'portfolio-grid-gutter' );
$infinite = LendizFamework::lendiz_static_theme_mod( 'portfolio-infinite-scroll' ) ? 'true' : 'false';
$isotope = LendizFamework::lendiz_static_theme_mod( 'portfolio-grid-type' );
$extra_class .= LendizFamework::lendiz_static_theme_mod( 'portfolio-grid-type' ) == 'normal' ? ' grid-normal' : '';
?>
		...
		<main id="main" class="site-main archive-template <?php echo esc_attr( $extra_class ); ?>" role="main" data-cols="<?php echo esc_attr( $cols ); ?>" data-gutter="<?php echo esc_attr( $gutter ); ?>">
			...
			if ( $query->have_posts() ) {
				...
				while ( $query->have_posts() ) : $query->the_post();
					...
					<article id="post-<?php the_ID(); ?>" <?php post_class( 'post lendiz-portfolio' ); ?>>
						<div class="portfolio-archive">
							<a href="<?php the_permalink(); ?>"><?php the_post_thumbnail( 'medium', array( 'class' => 'img-fluid' ) ); ?></a>
							<div class="portfolio-archive-title">
								<?php 
									$custom_url = get_post_meta( get_the_ID(), 'lendiz_portfolio_custom_url', true );
									$target = get_post_meta( get_the_ID(), 'lendiz_portfolio_custom_url_target', true );
